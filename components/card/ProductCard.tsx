...
export default function ProductCard({ img, name, price, id }: Props) {
    ...
    return (
        <>
            <View style={tw`z-0 flex-grow cursor-pointer justify-center box-border flex-row w-40 max-w-[10rem] md:max-w-[13rem] md:w-52 md:shadow-md border-yellow-400 rounded-lg border shadow-gray-300 transition ease-in-out hover:border-violet-700 delay-100 hover:-translate-y-1 hover:scale-110 duration-300`} onClick={() => window.location.href="/item/" + id}>
                <img src={img} style={tw`p-4 w-40 h-40 md:h-52 md:w-52 rounded-t-md`} />
